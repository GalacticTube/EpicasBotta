...
print(crayons.cyan(f'[Forter] [{time()}] Forter made by NJ3ZNAY0MY '
...
async def event_ready() -> None:
    print(crayons.green(f'[Forter] [{time()}] Forter logged in as {client.user.display_name}.'))
    ...
    for pending in list(client.pending_friends.values()):
        if pending.direction == 'INBOUND':
            ...
            if isinstance(friend, fortnitepy.Friend):
                print(f"[Forter] [{time()}] Accepted friend request from: {friend.display_name}.")
            else:
                print(f"[Forter] [{time()}] Declined friend request from: {pending.display_name}.")


@client.event
async def event_party_invite(invite: fortnitepy.PartyInvitation) -> None:
    await invite.accept()
    print(f'[Forter] [{time()}] Accepted party invite from {invite.sender.display_name}.')


@client.event
async def event_friend_request(request: fortnitepy.PendingFriend) -> None:
    print(f"[Forter] [{time()}] Received friend request from: {request.display_name}.")

    if data['friendaccept']:
        await request.accept()
        print(f"[Forter] [{time()}] Accepted friend request from: {request.display_name}.")
    else:
        await request.decline()
        print(f"[Forter] [{time()}] Declined friend request from: {request.display_name}.")
...
async def event_friend_message(message: fortnitepy.FriendMessage) -> None:
    ...
    print(f'[Forter] [{time()}] {message.author.display_name}: {message.content}')
    ...
    if "!skin" in args[0].lower():
        try:
            ...
            print(f"[Forter] [{time()}] Set skin to: {cosmetic.id}.")
            ...
            print(f"[Forter] [{time()}] Couldn't find a skin with the name: {content}.")
        ...
        try:
            ...
            print(f"[Forter] [{time()}] Set backpack to: {cosmetic.id}.")
            ...
            print(f"[Forter] [{time()}] Couldn't find a backpack with the name: {content}.")
        ...
        try:
            ...
            print(f"[Forter] [{time()}] Set emote to: {cosmetic.id}.")
            ...
            print(f"[Forter] [{time()}] Couldn't find an emote with the name: {content}.")
        ...
        try:
            ...
            print(f"[Forter] [{time()}] Set pickaxe to: {cosmetic.id}.")
            ...
            print(f"[Forter] [{time()}] Couldn't find a pickaxe with the name: {content}.")
        ...
        try:
            ...
            print(f"[Forter] [{time()}] Couldn't find an emoji with the name: {content}.")
        ...
        try:
            ...
            print(f"[Forter] [{time()}] Set contrail to: {cosmetic.id}.")
            ...
            print(f"[Forter] [{time()}] Couldn't find an contrail with the name: {content}.")
        ...
        print(f"[Forter] [{time()}] Banner set to: {args[1]}, {args[2]}, {args[3]}.")
        ...
        print(f'[Forter] [{time()}] Skin set to {args[0]}')
        ...
        print(f'[Forter] [{time()}] Set variants of {args[1]} to {args[2]} {args[3]}.')
        ...
        print(f'[Forter] [{time()}] Status set to {content}.')
        ...
        print(f'[Forter] [{time()}] Left the party as I was requested.')
        ...
        if member is None:
            ...
            try:
                ...
                print(f"[Forter] [{time()}] Kicked user: {member.display_name}")
            except fortnitepy.Forbidden:
                await message.reply(f"Couldn't kick {member.display_name}, as I'm not party leader.")
                print(crayons.red(f"[Forter] [{time()}] [ERROR] "
        ...
        if member is None:
            ...
            try:
                ...
                print(f"[Forter] [{time()}] Promoted user: {member.display_name}")
            except fortnitepy.Forbidden:
        ...
        try:
            ...
            print(f'[Forter] [{time()}] Party privacy set to {client.user.party.privacy}.')
...
if (data['email'] and data['password']) and (data['email'] != 'email@email.com' and data['password'] != 'password1'):
    try:
        ...
        print(crayons.red(f"[Forter] [{time()}] [ERROR] {e}"))
else:
    print(crayons.red(f"[Forter] [{time()}] [ERROR] Failed to login as no (or default) account details provided."))
